...
import { makeKeyId } from "./lib/keyring";
...
export default function App() {
  ...
  async function resetAccountData() {
    if (!userId) return;
    const provider = useWebAuthn ? "webauthn" : "password";
    try {
  ...
  // Reactive vault existence via live query (resilient to transient empties)
  // NOTE: Some @powersync/react setups briefly emit an empty result set during
  // fetching/back navigation. We smooth that out by caching the last non-empty
  // rows and using them while a fetch is in-flight to avoid UI flicker.
  const { data: vaultRows = [], isFetching: vaultFetching } = useQuery(
    "SELECT provider FROM e2ee_keys WHERE user_id = ?",
    [userId ?? ""],
    { throttleMs: 50 },
  );
  const [lastVaultRows, setLastVaultRows] = useState<any[]>([]);
  useEffect(() => {
    if (!userId) {
    ...
    try {
      ...
      // Fix for flaky behaviour: when fetching and rows are [] temporarily,
      // fall back to lastVaultRows so header/buttons don't bounce.
      const rowsStable =
        vaultFetching && rows.length === 0 ? lastVaultRows : rows;
      if (!vaultFetching && rowsStable !== lastVaultRows)
        setLastVaultRows(rowsStable);
      const providers = new Set(rowsStable.map((r: any) => r.provider));
  ...
  }, [vaultRows, vaultFetching, userId, lastVaultRows]);
